...
async def process_single_miner_response(
    ...
    try:
        ...
        qa_score_task = get_qa_score(compressed_context, questions, ground_truths, model)
        existence_score, qa_score = await asyncio.gather(existence_score_task, qa_score_task)
...
async def get_metrics(item: BatchedScoringRequest):
    ...
    for i, (existence_score, qa_score) in enumerate(results):
        if existence_score is None or qa_score is None or np.isnan(existence_score) or np.isnan(qa_score):
...
"""
            ...
            return "yes" in text.split() and not ("no" in text.split() or "not" in text.split())
        except Exception as e:
...
"""
    ...
    positive_scores, negative_scores = await asyncio.gather(
        asyncio.gather(*positive_tasks),
        asyncio.gather(*negative_tasks)
    )
