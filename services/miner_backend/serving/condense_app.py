...
class Condenser(nn.Module):
    ...
    def __init__(self,num_condense_tokens):
        ...
        self.condense_model = AutoModelForCausalLM.from_pretrained("Condense-AI/Condenser-Llama-3.2-1B-20241117-173040", torch_dtype=self.dtype).to("cuda")
        self.condense_tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-1B")
        ...
        self.norm = nn.LayerNorm(self.hidden_size * n_last_hidden_states).to(dtype=self.dtype, device="cuda")
        self.linear = nn.Linear(self.hidden_size * n_last_hidden_states, 4096).to(dtype=self.dtype, device="cuda")
        self.pre_condensed_tokens = nn.Parameter(torch.randn(1, num_condense_tokens, self.hidden_size, dtype=self.dtype, device="cuda"))
    ...
    def compress(self, context: str) -> torch.Tensor:
        ...
        expanded_attention_mask = torch.cat([attention_mask, torch.ones(attention_mask.shape[0], self.num_condense_tokens, dtype=attention_mask.dtype, device=attention_mask.device)], dim=1)
        
        # Generate condensed tokens
        output = self.condense_model(inputs_embeds=inputs_embeds_condense, output_hidden_states=True, attention_mask=expanded_attention_mask)
        hidden_states = torch.cat(output.hidden_states[-self.n_last_hidden_states:], dim=-1)[:, -self.num_condense_tokens:, :]
...
file_path = huggingface_hub.hf_hub_download(repo_id="Condense-AI/Condenser-Llama-3.2-1B-20241117-173040", filename="checkpoints/modules.pt", local_dir="./")
